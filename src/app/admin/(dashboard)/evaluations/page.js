...
export default function AdminEvaluations() {
  ...
  const fetchEvaluations = async (pageNum, append, searchTerm, status, level) => {
  ...
  const handleSearch = (e) => {
    ...
    searchTimeoutRef.current = setTimeout(() => {
      setPage(1);
      fetchEvaluations(1, false, value, statusFilter, levelFilter);
    }, 400);
  };

  // ✅ Status filter — single fetch
  const handleStatusFilter = (e) => {
    const value = e.target.value;
    setStatusFilter(value);
    setPage(1);
    fetchEvaluations(1, false, searchInput, value, levelFilter);
  };
  ...
  const handleLevelFilter = (e) => {
    ...
    setPage(1);
    fetchEvaluations(1, false, searchInput, statusFilter, value);
  };
  ...
  const loadMore = () => {
    ...
    const next = page + 1;
    setPage(next);
    fetchEvaluations(next, true, searchInput, statusFilter, levelFilter);
  };
